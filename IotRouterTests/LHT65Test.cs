...
public class LHT65Test
{
    [Fact]
    public void Test()
    {
        var serviceProviderMock = new Mock<IServiceProvider>();
        var logger = Mock.Of<ILogger<LHT65>>();
        serviceProviderMock.Setup(m => m.GetService(It.IsAny<Type>())).Returns(logger);

        var parser = new LHT65(serviceProviderMock.Object, null, "test");

        var rawJson = """
        { "end_device_ids": { "dev_eui": "eui" }, "uplink_message": { "f_port": 2, "received_at": "2024-08-09T14:14:13.093129857Z", "frm_payload": "y+UM3gK5AX//f/8=", "decoded_payload": "", "rx_metadata": [ {"rssi": "-81" } ] } }
        """;
        ...
        Assert.Equal(3.045m, (decimal)keyValues["batV"]);
        Assert.Equal(69.7m, (decimal)keyValues["humPrc"]);
        Assert.Equal(32.94m, (decimal)keyValues["tempC"]);
