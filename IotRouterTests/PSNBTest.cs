...
public class PSNBTest
{
    ...
    public void Test()
    {
        ...
        Assert.Equal("F860631074843179", result.DevEUI);
        Assert.Equal(0, result.FPort);
        Assert.Equal(639008101980000000, result.DateTime?.Ticks);
        var keyValues = result.KeyValues.ToDictionary(kv => kv.Key, kv => kv.Value);        
        Assert.Equal(4.002m, (decimal)keyValues["pressure"]);
        Assert.Equal(0.625m, (decimal)keyValues["distance"]);
        Assert.Equal(3.62m, (decimal)keyValues["batV"]);
        Assert.Equal(-97m, (decimal)keyValues["RSSI"]);
    }
    ...
    public void Test2()
    {
        ...
        Assert.Equal("F860631074843179", result.DevEUI);
        Assert.Equal(0, result.FPort);
        Assert.Equal(639008194920000000, result.DateTime?.Ticks);
        var keyValues = result.KeyValues.ToDictionary(kv => kv.Key, kv => kv.Value);        
        Assert.Equal(4.437m, (decimal)keyValues["pressure"]);
        Assert.Equal(136.5625m, (decimal)keyValues["distance"]);
        Assert.Equal(3.617m, (decimal)keyValues["batV"]);
