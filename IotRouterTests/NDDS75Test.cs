...
public class NDDS75Test
{
    ...
    public void Test()
    {
        ...
        Assert.Equal(638357654600000000, result.DateTime?.Ticks);
        var keyValues = result.KeyValues.ToDictionary(kv => kv.Key, kv => kv.Value);        
        Assert.Equal(744m, (decimal)keyValues["distance"]);
        Assert.Equal(3.350m, (decimal)keyValues["batV"]);
